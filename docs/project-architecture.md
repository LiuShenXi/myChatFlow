...
认证边界先经过 middleware，再在路由内部二次校验：

- `/api/chat`
- `/api/sessions`
- `/api/keys`

这样可以把主链路接口的入口保护前置，而路由本身仍保留 `auth()` 校验作为业务级兜底。
...
去推断或修正能力状态。对已知或可识别的兼容路径，系统会在运行中结合 provider hint、模型信息和实际调用反馈逐步修正能力标记；如果图片输入失败，才会在这些明确可归因的路径上做降级，避免把这个行为泛化到所有自定义模型。
...
后续扩展仍应优先沿着 provider、会话、密钥、自定义模型和消息流这些既有边界推进。
