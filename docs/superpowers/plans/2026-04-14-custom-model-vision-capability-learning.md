...
本计划在执行过程中做过一次交互策略调整，最终代码行为与早期计划存在一处关键差异：

- 计划早期版本：`unknown` 默认不放图
- 最终落地版本：`unknown` 默认允许尝试图片请求；若真实调用命中高置信度“非视觉模型”错误，再自动学习为 `text-only`

调整原因：

- 纯保守拦截会让可用的视觉模型首次使用门槛过高
- 用户侧更希望先试一次，再由系统自动学习并关闭不支持图片的模型
