...
export function modelSupportsImageInput(selectedModelId: string) {
  if (isCustomModelId(selectedModelId)) {
    return true
  }
  ...
  return (
    AVAILABLE_MODELS.find(
      (model) =>
        model.id === selectedModelId || model.modelId === selectedModelId
    )?.supportsVision ?? false
  )
}
